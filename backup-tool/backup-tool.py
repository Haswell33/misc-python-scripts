...
class Backup:
    ...
    def create(self):  # TO DO (archiving directories)
        os.system(self.cmd)
        self.set_privileges()
        if self.format == TAR_FORMAT:
            with tarfile.open(self.dest_dir, 'w:gz') as tar:
                tar.add(self.dest_dir, arcname=os.path.basename(self.dest_dir) + '.tar')
...
class FileBackup(Backup):
    ...
    def set_dest_dir(self):
        return self.create_dir(os.path.join(self.parent_dir, f'backup-{get_today()}'))
...
class DatabaseBackup(Backup):
    ...
    def set_dest_dir(self):
        return self.create_dir(os.path.join(self.parent_dir, self.database, f'backup-{get_today()}'))
...
def parse_args():
    parser = argparse.ArgumentParser(prog='',
                                     description='Script which using rsync-backup to make backups. It is rsync with --archive mode, '
                                                 'but with some additionally functions',
                                     epilog='')
    ...
    parser.add_argument('-n', '--nopasswd',
                        help='Force to not use password and not prompt for password, script will try to lookup to default files with passwords',
                        action='store_true')
    ...
    parser.add_argument('--daemonPasswdFile',
                        help=f'File with password for rsync user to establish connection in rsync by daemon, '
                             f'default is {DEFAULTS["PASSWD_FILE"]["DAEMON"]}',
                        type=str,
                        metavar='file',
                        default=DEFAULTS['PASSWD_FILE']['DAEMON'])
    parser.add_argument('--sshPasswdFile',
                        help=f'File with password for rsync user to establish connection in rsync via ssh, '
                             f'default is {DEFAULTS["PASSWD_FILE"]["SSH"]}',
                        type=str,
                        metavar='file',
                        default=DEFAULTS['PASSWD_FILE']['SSH'])
    parser.add_argument('--hostsFile',
                        help=f', '
                             f'default is {DEFAULTS["HOSTS_FILE"]}',
                        type=str,
                        metavar='file',
    ...
    try:
        ...
        if args.type in DEFAULTS['DATABASE_BACKUP_CHOICES']:
            for database in args.databases:
                backup = DatabaseBackup(args.type, host, args.user, args.password, args.nopasswd, args.destDir, args.max, args.owner, args.format, database, args.dbPort)
